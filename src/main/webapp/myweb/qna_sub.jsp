...
<table id="qnatable">
 ...
 <c:forEach var = "q" items="${list }">
  ...
  <tr>
  <td>${q.num }</td>
  <td align="left" style="font-size: 13px;"><c:if test="${q.picture != ' ' }"> <img src="image/${q.picture }" 
 ...
 <tr>
  <td colspan="8" align="center">
   <c:if test="${page.curPage > 1}">
    <a href="#" onclick="table3('1')">[처음]</a>
   </c:if>
   <c:if test="${page.curBlock > 1}">
    <a href="#" onclick="table3('${page.prevPage}')">[이전]</a>
   </c:if>
   <c:forEach var="num" begin="${page.blockStart}" end="${page.blockEnd}">
    <c:choose>
     ...
     <c:otherwise>
      <a href="#" onclick="table3('${num}')">${num}</a>
     </c:otherwise>
    </c:choose>
   </c:forEach>
   <c:if test="${page.curBlock < page.totBlock}">
    <a href="#" onclick="table3('${page.nextPage}')">[다음]</a>
   </c:if>
   <c:if test="${page.curPage < page.totPage}">
    <a href="#" onclick="table3('${page.totPage}')">[끝]</a>
   </c:if>
  </td>
 </tr>
</table>
